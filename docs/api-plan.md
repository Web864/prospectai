...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Session                 | `GET /me`, `PATCH /me`, `POST /logout`                                                                                                                  | Current user, profile update, or web session termination; signed-in user.           | Profile mutation is validated. Password and Google PKCE callbacks use the implemented server session service.                                             |
...
| Pitches                 | `POST /pitches`, `GET /pitches/:id`, `PATCH /pitches/:id`                                                                                               | Generate and retain an editable outreach pitch for a lead or analysis.              | Requires eligible analysis evidence and a configured AI provider; malformed or ungrounded provider output is rejected.                                    |
...
The Stripe endpoint verifies the provider signature against the unparsed request body and persists the provider event id for replay protection before applying subscription state. Stripe retries failed delivery. The endpoint never trusts a frontend checkout redirect as payment truth.
...
## Value-First Guest API Addendum

The earlier authenticated-only analysis entry is superseded for extension first use.

| Endpoint                       | Authorization                                | Contract and controls                                                                                                                                    |
| ------------------------------ | -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /guest-sessions`         | `Authorization: Guest <opaque-token>`        | Idempotently establishes a hash-only anonymous session and returns server-authoritative entitlement state. Creation must be rate-limited before release. |
| `GET /guest-sessions`          | Active guest bearer                          | Returns `trialLimit`, `trialUsed`, `trialRemaining`, status, and expiry; never trusts a client counter.                                                  |
| `POST /guest-analyses`         | Active guest bearer plus `Idempotency-Key`   | Validates a supported public URL, serializes allowance, creates analysis/job, and reserves one guest use atomically.                                     |
| `GET /analysis-jobs/:id`       | Owning guest bearer or authenticated tenant  | Returns progress only within the caller's guest or tenant boundary.                                                                                      |
| `GET /guest-analyses/:id`      | Owning guest bearer                          | Returns only basic summary, Opportunity Score/reasoning, key signals, and next action.                                                                   |
| `POST /guest-sessions/convert` | Authenticated user plus matching guest token | Transactional and idempotent transfer of eligible guest analyses and active jobs.                                                                        |

Errors add `GUEST_TRIAL_EXHAUSTED`, `GUEST_SESSION_INVALID`, `GUEST_SESSION_EXPIRED`, `GUEST_CONVERSION_FAILED`, `AUTH_HANDOFF_EXPIRED`, and `AUTH_HANDOFF_INVALID`. Exhaustion is an entitlement state, not `ANALYSIS_FAILED`.

### Guest Contract Types

The shared contract layer exports GuestSession, GuestEntitlement, GuestUsage, GuestAnalysisRequest, GuestAnalysisResult, GuestConversion, AuthHandoff, and TrialRemaining. Guest and registered analysis share the same PostgreSQL job state contract; only authorization and result visibility differ. Save Lead, full pitch/history, and registered usage remain authenticated endpoints and trigger progressive registration from guest UI.
